"use strict";
const components_mpHtml_editable_config = require("./config.js");
...
function Editable(vm) {
  ...
  vm._getItem = function(node, up, down) {
    ...
    let i;
    if (node === "color") {
      return components_mpHtml_editable_config.config.color;
    }
    if (node.name === "img") {
      items = components_mpHtml_editable_config.config.img.slice(0);
      if (!vm.getSrc) {
        i = items.indexOf("换图");
        if (i !== -1) {
          items.splice(i, 1);
      ...
      items = components_mpHtml_editable_config.config.link.slice(0);
      if (!vm.getSrc) {
      ...
      items = components_mpHtml_editable_config.config.media.slice(0);
      i = items.indexOf("封面");
      if (!vm.getSrc && i !== -1) {
        items.splice(i, 1);
      }
      ...
      items = components_mpHtml_editable_config.config.card.slice(0);
    } else {
      items = components_mpHtml_editable_config.config.node.slice(0);
    }
    if (!up) {
      i = items.indexOf("上移");
      if (i !== -1) {
  ...
  vm.insertHtml = (html) => {
    this.inserting = true;
    const arr = new components_mpHtml_parser.Parser(vm).parse(html);
  ...
  vm.insertImg = function() {
    vm.getSrc && vm.getSrc("img").then((src) => {
      ...
      const parser = new components_mpHtml_parser.Parser(vm);
